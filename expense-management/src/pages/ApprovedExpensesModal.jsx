...
const ApprovedExpensesModal = ({ onClose }) => {
  ...
  const fetchApprovedExpenses = async () => {
    try {
      const response = await api.get('/expenses/approved');
      setExpenses(response.data);
      setError('');
    } catch (error) {
      setError(error.response?.data?.message || 'Failed to fetch approved expenses');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full">
      <div className="relative top-20 mx-auto p-5 border w-3/4 shadow-lg rounded-md bg-white">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-medium">Approved Expenses</h3>
          <button
            onClick={onClose}
